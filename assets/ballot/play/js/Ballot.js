...
function ScoreBallot(config){
	...
	var self = this;
	config = config || {};
	config.bg = "/assets/ballot/play/img/ballot_range.png";
...
function ApprovalBallot(config){
	...
	var self = this;
	config = config || {};
	config.bg = "/assets/ballot/play/img/ballot_approval.png";
	Ballot.call(self, config);
...
function RankedBallot(config){
	...
	var self = this;
	config = config || {};
	config.bg = "/assets/ballot/play/img/ballot_ranked.png";
	Ballot.call(self, config);
...
function PluralityBallot(config){
	...
	var self = this;
	config = config || {};
	config.bg = "/assets/ballot/play/img/ballot_fptp.png";
	Ballot.call(self, config);
...
function Ballot(config){
	...
	self.createBox = function(x,y,frame){
		return self.createSprite({
			img: "/assets/ballot/play/img/ballot_box.png",
			x:x, y:y,
			w:50, h:50,
			frame:frame
		});
	...
	self.createRate = function(x,y,frame){
		return self.createSprite({
			img: "/assets/ballot/play/img/ballot_rate.png",
			x:x, y:y,
			w:225, h:50,
			frame:frame
		});
